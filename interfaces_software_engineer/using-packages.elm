...
```bash
elm install elm-lang/html         # Install latest version
elm install elm-lang/html 1.0.0   # Install version 1.0.0
```

`elmd-package` is sandboxed by default, so the downloaded package will be placed
...
```bash
elm diff elm-lang/core 3.0.0 4.0.0
```
...
If you like what you see, take the following steps.
  ...
  * Run `elm install elm-lang/core 4.0.0` and see how things go!
...
```bash
elm diff        # diff current API with most recently published API
elm diff 1.0.0  # diff current API with version 1.0.0
```
